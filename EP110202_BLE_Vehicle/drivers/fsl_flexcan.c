...
/*!
 * @brief Sub Handler Mail Box Trasfered Events
 *
 * @param base FlexCAN peripheral base address.
 ...
 */
static status_t FLEXCAN_SubHandlerForMailBoxTransfered(CAN_Type *base, flexcan_handle_t *handle, uint32_t mbIdx);
...
/* Reset MCR register. */
    ...
    base->MCR |= CAN_MCR_WRNEN_MASK | CAN_MCR_WAKSRC_MASK |
                 CAN_MCR_MAXMB((uint32_t)FSL_FEATURE_FLEXCAN_HAS_MESSAGE_BUFFER_MAX_NUMBERn(base) - 1UL);
#else
...
void FLEXCAN_Init(CAN_Type *base, const flexcan_config_t *config, uint32_t sourceClock_Hz)
{
    ...
    base->CTRL1 |= CAN_CTRL1_BOFFREC(1);//add by lyp  Bus off后取消发送

    mcrTemp |= CAN_MCR_SRXDIS_MASK;//add by shawn, ��Ҫ�ֶ���Ӵ����ֹ�Է����գ�Ĭ���Ǵ򿪵�
    ...
    CAN_Type *base, const flexcan_config_t *config, uint32_t sourceClock_Hz, flexcan_mb_size_t dataSize, bool brs)
{
    ...
    {
        /* The TDC offset should be configured as shown in this equation : (FPSEG1 + FPROPSEG + 2) * (FPRESDIV + 1) */
        if (((uint32_t)config->timingConfig.fphaseSeg1 + config->timingConfig.fpropSeg + 2U) *
                (config->timingConfig.fpreDivider + 1) <
            MAX_TDCOFF)
        {
            fdctrl =
                (fdctrl & ~CAN_FDCTRL_TDCOFF_MASK) |
                CAN_FDCTRL_TDCOFF(((uint32_t)config->timingConfig.fphaseSeg1 + config->timingConfig.fpropSeg + 2U) *
                                  (config->timingConfig.fpreDivider + 1));
        }
        else
        {
 ...
 * param mbIdx The FlexCAN Message Buffer index.
 */
bool FLEXCAN_IsMbOccupied(CAN_Type *base, uint8_t mbIdx)
{
    uint8_t lastOccupiedMb;

    /* Is Rx FIFO enabled? */
    if ((base->MCR & CAN_MCR_RFEN_MASK) != 0UL)
    {
        /* Get RFFN value. */
...
void FLEXCAN_SetTxMbConfig(CAN_Type *base, uint8_t mbIdx, bool enable)
{
    ...
    assert(FLEXCAN_IsMbOccupied(base, mbIdx) == false);
...
{
    uint32_t ideal_sp;
    uint32_t time_seg1, time_seg2;
    bool fgRet = false;
    assert(tqNum <= CTRL1_MAX_TIME_QUANTA);
    ...
    time_seg1 = (tqNum * ideal_sp) / (uint32_t)IDEAL_SP_FACTOR - 1U;
    if ((tqNum - time_seg1 - 1U) < MIN_TIME_SEGMENT2)
    {
        time_seg2 = MIN_TIME_SEGMENT2;
        time_seg1 = tqNum - time_seg2 - 1U;
    }
    else
    {
        time_seg2 = tqNum - time_seg1 - 1U;
    }

    if ((time_seg1 <= (MAX_PSEG1 + MAX_PROPSEG + 2U)) && (time_seg1 >= MIN_TIME_SEGMENT1))
    {
        if (time_seg1 > (MAX_PROPSEG + 1U))
        {
            pTimingConfig->phaseSeg1 = time_seg1 - MAX_PROPSEG - 2U;
            pTimingConfig->propSeg   = MAX_PROPSEG;
        }
        else
        {
            pTimingConfig->phaseSeg1 = 0U;
            pTimingConfig->propSeg   = time_seg1 - 2U;
        }

        /* The value of prog Seg should be not larger than tqNum - 3U. */
        if (time_seg1 <= (tqNum - 3U))
        {
            pTimingConfig->phaseSeg2 = time_seg2 - 1U;
            ...
            if (pTimingConfig->phaseSeg2 <= MAX_PSEG2)
            {
                ...
                /* The max tqNum for CTRL1 will reach to 25, ERJW would not be larger than 4. */
                /* Considering that max ERJW is 3, rJumpwidth will always be smaller than MAX_ERJW. */
...
bool FLEXCAN_CalculateImprovedTimingValues(uint32_t baudRate, uint32_t sourceClock_Hz, flexcan_timing_config_t *pconfig)
{
    ...
    do
    {
        ...
        if (pconfig->preDivider > MAX_PRESDIV)
        {
        ...
        if (FLEXCAN_GetSegments(baudRate, tqNum, pconfig))
        {
            fgRet = true;
            break;
        }
...
static bool FLEXCAN_FDGetSegments(uint32_t baudRate, uint32_t tqNum, flexcan_timing_config_t *pTimingConfig)
{
    ...
    uint32_t time_seg1, time_seg2;
    bool fgRet = false;
    assert(tqNum <= CBT_MAX_TIME_QUANTA);
    ...
    if ((time_seg1 <= (MAX_EPSEG1 + MAX_EPROPSEG + 2U)) && (time_seg1 >= MIN_TIME_SEGMENT1))
    {
        if (time_seg1 > (MAX_EPROPSEG + 1U))
        {
            pTimingConfig->phaseSeg1 = time_seg1 - MAX_EPROPSEG - 2U;
...
static bool FLEXCAN_FDGetSegmentswithBRS(uint32_t baudRatebrs, uint32_t tqNum, flexcan_timing_config_t *pTimingConfig)
{
    ...
    assert(tqNum <= FDCBT_MAX_TIME_QUANTA);
    ...
    if ((time_seg1 <= (MAX_FPSEG1 + MAX_FPROPSEG + 1U)) && (time_seg1 >= MIN_TIME_SEGMENT1))
    {
        if (time_seg1 > MAX_FPROPSEG)
        {
            pTimingConfig->fphaseSeg1 = time_seg1 - MAX_FPROPSEG - 1U;
            pTimingConfig->fpropSeg   = MAX_FPROPSEG;
        }
        else
        {
            pTimingConfig->fphaseSeg1 = 0U;
            pTimingConfig->fpropSeg   = time_seg1 - 1U;
        }

        /* The value of time_seg1 should be not larger than tqNum -3U. */
        if (time_seg1 <= (tqNum - 3U))
        {
            pTimingConfig->fphaseSeg2 = time_seg2 - 1U;
...
void FLEXCAN_SetRxFifoConfig(CAN_Type *base, const flexcan_rx_fifo_config_t *config, bool enable)
{
    ...
    volatile uint32_t *idFilterRegion = (volatile uint32_t *)(void *)(&base->MB[6]);
    uint8_t i, rffn = 0;
    uint32_t setup_mb = 0;
...
status_t FLEXCAN_TransferSendNonBlocking(CAN_Type *base, flexcan_handle_t *handle, flexcan_mb_transfer_t *xfer)
{
    ...
    assert(FLEXCAN_IsMbOccupied(base, xfer->mbIdx) == false);
...
void FLEXCAN_TransferAbortSend(CAN_Type *base, flexcan_handle_t *handle, uint8_t mbIdx)
{
    ...
    uint16_t timestamp;
#if 1 
    if (NULL == handle->mbFrameBuf[mbIdx])
    {
    	return;
    }
    /* Assertion. */
...
/* Disable Message Buffer Interrupt. */
    ...
    handle->mbFrameBuf[mbIdx]->timestamp = timestamp;
...
/* Disable Message Buffer Interrupt. */
    ...
    handle->mbFDFrameBuf[mbIdx]->timestamp = timestamp;
...
static status_t FLEXCAN_SubHandlerForMailBoxTransfered(CAN_Type *base, flexcan_handle_t *handle, uint32_t mbIdx)
{
    ...
    switch (handle->mbState[mbIdx])
    {
        ...
        case (uint8_t)kFLEXCAN_StateRxRemote:
            ...
            {
                status = FLEXCAN_ReadFDRxMb(base, (uint8_t)mbIdx, handle->mbFDFrameBuf[mbIdx]);
            }
            else
            {
                status = FLEXCAN_ReadRxMb(base, (uint8_t)mbIdx, handle->mbFrameBuf[mbIdx]);
            }
#else
            status = FLEXCAN_ReadRxMb(base, mbIdx, handle->mbFrameBuf[mbIdx]);
#endif
            ...
            if (0U != (base->MCR & CAN_MCR_FDEN_MASK))
            {
                FLEXCAN_TransferFDAbortReceive(base, handle, (uint8_t)mbIdx);
            }
            else
            {
                FLEXCAN_TransferAbortReceive(base, handle, (uint8_t)mbIdx);
            }
#else
            FLEXCAN_TransferAbortReceive(base, handle, mbIdx);
#endif
            break;
        ...
        case (uint8_t)kFLEXCAN_StateTxData:
            ...
            if (0U != (base->MCR & CAN_MCR_FDEN_MASK))
            {
                FLEXCAN_TransferFDAbortSend(base, handle, (uint8_t)mbIdx);
            }
            else
            {
                FLEXCAN_TransferAbortSend(base, handle, (uint8_t)mbIdx);
            }
#else
            FLEXCAN_TransferAbortSend(base, handle, mbIdx);
#endif
            break;

        /* Solve Tx Remote Frame. */
        case (uint8_t)kFLEXCAN_StateTxRemote:
            handle->mbState[mbIdx] = (uint8_t)kFLEXCAN_StateRxRemote;
...
void FLEXCAN_TransferHandleIRQ(CAN_Type *base, flexcan_handle_t *handle)
{
    ...
    do
    {
                                ...
                                (uint32_t)kFLEXCAN_BusOffIntFlag | (uint32_t)kFLEXCAN_ErrorIntFlag)))
        {
            ...
            // if(0U != (EsrStatus & kFLEXCAN_BusOffIntFlag))
            // {
            // 	result = EsrStatus & kFLEXCAN_BusOffIntFlag;
            // }
