...
static const bootInfo_t gBootFlags =
...
#endif
{
    ...
    {gBootValueForTRUE_c, 0, 0, 0, 0, 0, 0, 0},
...
otaResult_t OTA_CommitImage(uint8_t* pBitmap)
{
    ...
    else
    {
        ...
        const uint8_t start_marker[gBootData_Marker_Size_c] = {gBootData_StartMarker_Value_c};

        /* Write the Start marker at the beginning of the internal storage. */
        if(EEPROM_WriteData(gBootData_Marker_Size_c, gBootData_StartMarker_Offset_c, (uint8_t*)&start_marker[0]) != ee_ok)
        {
            status = gOtaExternalFlashError_c;
        }
        else
