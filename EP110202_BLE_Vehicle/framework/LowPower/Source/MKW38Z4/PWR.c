...
void PWR_HandleDeepSleepMode(void)
{
    ...
    * and save the spend time in ticks that wasn't counted.    */
    ...
    /* Clear potential wakeup reasons but do not call PWRLib_LLWU_UpdateWakeupReason() , the event will be taken from the interrupt,
        not from the PWRLib_MCU_WakeupReason variable                   */
...
static void PWR_HandleDeepSleepMode_1_2_3(void)
{
...
#endif
    {
        ...
        bool_t          abort           = FALSE;   // Set to False in case cPWR_BLE_LL_Enable not set
...
static void PWR_HandleDeepSleepMode_4(void)
{
    ...
    /* Restart lowpower timers and RTC if disabled during lowpower entry */
    BOARD_RTC_Init();
    PWRLib_LPTMR_Init();
    ...
    PWR_DBG_LOG("Early Wakeup due to late IT: %d", mode);

    /* Clear wakeup sources if any , event will be taken by IRQ */
    PWRLib_LLWU_ClearWakeUpSources();
    ...
    if(enterLowPower)
    {
        ...
        if (abort == FALSE)
        {
            ...
            * and save the spend time in ticks that wasn't counted.        */
            ...
            lowpower_skipped = PWRLib_MCU_Enter_vlls2_3();

            /* Reenable 32Mhz oscillator
              usually done in SystemInitHookWarm(), However, if the WFI has been skipped (no lowpower entry)
              It is mandatory to enable it again here for Radio ReInit and linkLayer wakeup */
            RSIM->CONTROL = RSIM_CONTROL_MODIFY_FIELD(RSIM_CONTROL_RF_OSC_EN, 1U);
            ...
            /* No need to reinitialize the peripherals if the lowpower has been skipped */
            if ( lowpower_skipped == FALSE )
            {
            ...
            /* Check if BLE LL exits DSM - Otherwise , force a wakeup now, this will take a while
               Warning : In case the device did not get to VLLS  (WFI skipped due to late interrupt)
                   wakeup of Link layer shall be requested after 32Mhz clock is ready */
            if(RSIM_CONTROL_READ_FIELD(RSIM_CONTROL_BLE_RF_OSC_REQ_INT) == 0U)
            {
                /* Radio will wait for 32Mhz during reinit so this can be done only
                     before SW request LL to wakeup */
                PWR_WaitFor32MhzReady();
            ...
            {
                 /* In case of early wakeup due to late interrupt, event will be handled directly from the ISR */
                 PWRLib_MCU_WakeupReason.AllBits = 0;
            }
...
void PWR_Init(void)
{
    ...
    warmbootConfig.doWarmbootSequence = 0x5A5A5A5AU;//WARMBOOT_SEQUENCE;
#else
