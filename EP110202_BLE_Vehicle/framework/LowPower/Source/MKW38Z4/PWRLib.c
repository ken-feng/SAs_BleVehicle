...
static void PWRLib_LLWU_UpdateWakeupReasonExt(uint32_t llwuFlags)
{
    ...
    if(llwuFlags & BOARD_LLWU_PIN_C2_BITMAP)
    {
        PWRLib_MCU_WakeupReason.Bits.FromKeyBoard = 1; 
    }
    if(llwuFlags & BOARD_LLWU_PIN_C4_BITMAP)
    {
        PWRLib_MCU_WakeupReason.Bits.FromCan = 1; 
    }
#if PTB0_IRQ_PIN
    if(llwuFlags & BOARD_LLWU_PIN_B0_BITMAP)
#else    
    if(llwuFlags & BOARD_LLWU_PIN_C16_BITMAP)
#endif    
    {
        PWRLib_MCU_WakeupReason.Bits.FromNFCIrq = 1; 
    }        
    ...
    if((llwuFlags & gPWRLib_LLWU_WakeupModuleFlag_RTC_Alarm_c) != 0U)
    {
        PWRLib_MCU_WakeupReason.Bits.FromRtcAlarm = 1;
    }

    if((llwuFlags & gPWRLib_LLWU_WakeupModuleFlag_RTC_Seconds_c) != 0U)
    {
        PWRLib_MCU_WakeupReason.Bits.FromRtcSeconds = 1;
    }
}
...
//#define LL_DEBUG_LP
#ifdef LL_DEBUG_LP
static void PWRLib_BLL_Debug(void)
{
    static volatile uint16_t dbg_event_status;
    static volatile uint16_t dbg_adv_next_instant;
    static volatile uint16_t dbg_adv_next_instant2;
    static volatile uint16_t dbg_nearest_instant;
    static volatile uint16_t dbg_adv_status;

    dbg_event_status           = BLE_LL_REG(EVENT_STATUS);
    dbg_adv_next_instant       = BLE_LL_REG(0x12);

    BLE_LL_REG(ENGINE_INDEX)   = 0x8;    // select other ADV engine
    dbg_adv_next_instant2      = BLE_LL_REG(0x12);
    BLE_LL_REG(ENGINE_INDEX)   = 0x0;    // select back to initial engine

    dbg_nearest_instant        = BLE_LL_REG(0x520);
    dbg_adv_status             = BLE_LL_REG(0x10);
}
#endif
 ...
 *---------------------------------------------------------------------------*/
bool_t PWRLib_BLL_EnterDSM(void)
{
    bool_t abort = FALSE;    /*  Don t abort by default */

#ifdef LL_DEBUG_LP
    PWRLib_BLL_Debug(void);
#endif

    PWR_DBG_LOG("%x", BLE_LL_REG(EVENT_STATUS));

    /* Abort the procedure if the LLH gets active again */
    if ( ( BLE_LL_REG_CLOCK_CONFIG & BLE_LL_CLOCK_CONFIG_LLH_IDLE_MASK ) == 0U )
    {
        abort = TRUE;
    }
    else
 ...
 *---------------------------------------------------------------------------*/
#if (cPWR_EnableDeepSleepMode_8 || cPWR_EnableDeepSleepMode_5 || cPWR_EnableDeepSleepMode_9)
bool PWRLib_MCU_Enter_vlls2_3(void)
{
    bool     lowpower_skipped = FALSE;
    uint32_t i;
    uint32_t scb_vtor;
    ...
    scb_shp[0] = SCB->SHP[0];
    scb_shp[1] = SCB->SHP[1];
    sysTCtrl = SysTick->CTRL;
    sysTLoad = SysTick->LOAD;
#endif
    sim_scgc5 = SIM->SCGC5;
    sim_sopt2 = SIM->SOPT2;
    ...
    if ( 0 == setjmp(pwr_CPUContext))
    {
        ...
        PWR_DBG_LOG("late IRQ wakeup mode=%d", mode);
    ...
    SCB->SHP[0] = scb_shp[0];
    SCB->SHP[1] = scb_shp[1];
    SysTick->CTRL = sysTCtrl;
    SysTick->LOAD = sysTLoad;
    SysTick->VAL = 0;
#endif
...
static void PWRLib_WaitFor32kOscRestart(void)
{
    ...
    /* Backup and disable all sources of wakeup, we need to make sure the wakeup is from the LPTMR counter
        being started from the 32Khz Oscillator */
    pe1 = LLWU->PE1;
    pe2 = LLWU->PE2;
    pe3 = LLWU->PE3;
    pe4 = LLWU->PE4;
    ...
    /* Restore the LLWU wakeup sources */
    LLWU->PE1 = pe1;
    LLWU->PE2 = pe2;
    LLWU->PE3 = pe3;
    LLWU->PE4 = pe4;
...
void PWRLib_Init(void)
{
    ...
    if((RCM_REG->SRS0 & RCM_SRS0_WAKEUP_MASK) != 0U)
    {
        ...
        if((RCM_REG->SRS1 & RCM_SRS1_SW_MASK) != 0U)
        {
            PWRLib_MCU_WakeupReason.Bits.FromSwReset = 1U;
        }
    }
