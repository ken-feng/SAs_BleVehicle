...
const unsigned char sm2_par_dig[128] = {
...
int core_algo_sm3_e(unsigned char *userid, int userid_len, unsigned char *xa, int xa_len, unsigned char *ya, int ya_len, unsigned char *msg, int msg_len, unsigned char *e)
{
/*
...
*/
	...
	buf = core_platform_alloc(2+userid_len+128+32+32);
#else   	
	buf = MEM_CallocAlt(2+userid_len+128+32+32,1);
#endif	
	if(buf == NULL)
		return -1;
	...
	core_mm_copy(buf+2, userid, userid_len);
	core_mm_copy(buf+2+userid_len, (u8*)sm2_par_dig, 128);

	core_mm_set(buf+2+userid_len+128, 0, 64);
	core_mm_copy(buf+2+userid_len+128+32-xa_len, xa, 32);
	core_mm_copy(buf+2+userid_len+128+32+32-ya_len, ya, 32);

	core_algo_sm3(buf, 2+userid_len+128+32+32, e);

	//free(buf);
#if EM_000101_CONFIG_FEATURE_RAM_MANAGEMENT   
    core_platform_free(buf);
#else 	
	MEM_FreeAlt(buf);
#endif
#if SM2_DEBUG
	printf("core_algo_sm3_e: ");
	PrintBuf(e, 32);
#endif

	//buf = malloc(msg_len+32);
#if EM_000101_CONFIG_FEATURE_RAM_MANAGEMENT   
	buf = core_platform_alloc(msg_len+32);
#else	
	buf = MEM_CallocAlt(msg_len+32,1);
#endif	
	if(buf == NULL)
		return -1;

	core_mm_copy(buf, e, 32);
	core_mm_copy(buf+32, msg, msg_len);
	core_algo_sm3(buf, 32+msg_len, e);
...
//               以下是P域上的ECC函数                   //
/*******************************************************/
void core_algo_sm2_keygen(unsigned char *wx, int *wxlen, unsigned char *wy, int *wylen,unsigned char *privkey, int *privkeylen)
{
/*
功能：生成SM2公私钥对
...
*/
	struct FPECC *cfig = (struct FPECC *)&Ecc256;
	epoint *g;
...
void core_algo_sm2_sign_withE(unsigned char *hash,int hashlen,unsigned char *privkey,int privkeylen,unsigned char *cr,int *rlen,unsigned char *cs,int *slen)
{
	/*
	功能：SM2签名
	[输入] hash：    core_algo_sm3_e()的结果
...
void core_algo_sm2_sign(unsigned char *src,int srclen,unsigned char *privkey,int privkeylen,unsigned char *wx,int wxlen, unsigned char *wy,int wylen,unsigned char *cr,int *rlen,unsigned char *cs,int *slen)
{
	/*
	功能：SM2签名
	[输入] src：    原文
	...
	[输出] slen：cs的字节数，32
	*/
	unsigned char usrid[16]={0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38,0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38};
    unsigned char hashbuff[32]={0};
	if(0 != core_algo_sm3_e(usrid,sizeof(usrid),wx,wxlen,wy,wylen,src,srclen,hashbuff))
	{
		return ;//-1;
	}
	core_algo_sm2_sign_withE(hashbuff,32,privkey,privkeylen,cr,rlen,cs,slen);
}
int  core_algo_sm2_verify_withE(unsigned char *hash,int hashlen,unsigned char  *cr,int rlen,unsigned char *cs,int slen, unsigned char *wx,int wxlen, unsigned char *wy,int wylen)
{
/*
功能：验证SM2签名
[输入] hash：    core_algo_sm3_e()的结果
...
*/
	...
	if(!epoint_set(x,y,0,w))
		goto exit_core_algo_sm2_verify;
...
int core_algo_sm2_verify(unsigned char *src,int srclen,unsigned char  *cr,int rlen,unsigned char *cs,int slen, unsigned char *wx,int wxlen, unsigned char *wy,int wylen)
{
	unsigned char usrid[16]={0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38,0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38};
    unsigned char hashbuff[32]={0};
	if(0 != core_algo_sm3_e(usrid,sizeof(usrid),wx,wxlen,wy,wylen,src,srclen,hashbuff))
	...
	return core_algo_sm2_verify_withE(hashbuff,32,cr,rlen,cs,slen,wx,wxlen,wy,wylen);
}
...
*/
    ...
    unsigned char* buf;
   	...
   	buf = core_platform_alloc(70);
   	digest = core_platform_alloc(32);
#else
	unsigned char buf[70];
	unsigned char digest[32];
#endif

	core_mm_copy(buf, zl, 32);
	core_mm_copy(buf+32, zr, 32);
	...
	if(n != 0)
	{
		...
		buf[66] = (ct >> 8) & 0xFF;
		buf[67] = ct & 0xFF;
		core_algo_sm3(buf, 68, digest);
	}

	core_mm_copy(p, digest, n);
...
int core_algo_sm2_encrypt(unsigned char *msg,int msglen, unsigned char *wx,int wxlen, unsigned char *wy,int wylen, unsigned char *outmsg)
{
/*
...
返回值：
		>0 :加密结果Len，比输入数据多96字节！outmsg+96，C1（64字节）和C3（32字节）保留前导0x00
		 0： 加密失败
*/
	struct FPECC *cfig = (struct FPECC *)&Ecc256;
    big x2, y2, c1, c2, k;
    ...
    unsigned char* tBuffer;
   	unsigned char* zl;
   	unsigned char* zr;
   	tBuffer = core_platform_alloc(32);
   	zl = core_platform_alloc(32);
   	zr = core_platform_alloc(32);
#else
    unsigned char tBuffer[32];
	unsigned char zl[32], zr[32];
#endif
	//tmp = malloc(msglen+64);
#if EM_000101_CONFIG_FEATURE_RAM_MANAGEMENT   
	tmp = core_platform_alloc(msglen+64+32);
#else		
    tmp = MEM_CallocAlt(msglen+64+32,1);
#endif	
	if(tmp == NULL)
		return 0;
...
core_algo_sm2_encrypt_again:
	...
	if(point_at_infinity(w))
		goto exit_core_algo_sm2_encrypt;
	...
	if (kdf(zl, zr, msglen, outmsg+64) == 0)
		goto core_algo_sm2_encrypt_again;
	//C1C2C3
	for(i = 0; i < msglen; i++)
	{
		outmsg[64+i] ^= msg[i];
	}
	core_mm_copy(tmp, zl, 32);
	core_mm_copy(tmp+32, msg, msglen);
	core_mm_copy(tmp+32+msglen, zr, 32);

	core_algo_sm3(tmp, 64+msglen, &outmsg[64+msglen]);

	//C1C2C3->C1C3C2  C1:64 Bytes C2:msglen Bytes C3:32 Bytes
	core_mm_copy(tBuffer, outmsg+64+msglen, 32);//C2
	core_mm_copy(outmsg+96, outmsg+64, msglen);//C2->C3
	core_mm_copy(outmsg+64, tBuffer, 32);//C3->C2

	ret = msglen+64+32;

exit_core_algo_sm2_encrypt:
	...
	//free(tmp);
#if EM_000101_CONFIG_FEATURE_RAM_MANAGEMENT   
    core_platform_free(tmp);
#else 	
	MEM_FreeAlt(tmp);
#endif
#if EM_000101_CONFIG_FEATURE_RAM_MANAGEMENT
	core_platform_free(zr);
   	core_platform_free(zl);
   	core_platform_free(tBuffer);
#endif
	return ret;
...
int core_algo_sm2_decrypt(unsigned char *msg,int msglen, unsigned char *privkey, int privkeylen, unsigned char *outmsg)
{
/*
功能：用SM2私钥解密数据。解密结果比输入数据少96字节！
[输入] msg     要解密的数据，是core_algo_sm2_encrypt()加密的结果，不少于96字节。
[输入] msglen：msg的字节数
[输入] privkey： 私钥
[输入] privkeylen： privkeylen的字节数
...
返回值：
		>0 :  解密结果Len，比输入数据多96字节！outmsg+96，C1（64字节）和C3（32字节）保留前导0x00
		 0： 解密失败
*/
	struct FPECC *cfig = (struct FPECC *)&Ecc256;
    big x2, y2, c, k;
	...
	//C1C3C2->C1C2C3  C1:64 Bytes   C2:msglen Bytes C3:32 Bytes
	core_mm_copy(tBuffer, msg+64, 32);//C3
	core_mm_copy(msg+64, msg+96, msglen);//C2->C3
	core_mm_copy(msg+64+msglen, tBuffer, 32);//C3->C2

	//tmp = malloc(msglen+64);
#if EM_000101_CONFIG_FEATURE_RAM_MANAGEMENT   
	tmp = core_platform_alloc(msglen+64);
#else		
	tmp = MEM_CallocAlt(msglen+64,1);
#endif	
	if(tmp == NULL)
		return 0;
	...
	core_mm_copy(tmp, zl, 32);
	core_mm_copy(tmp+32, outmsg, msglen);
	core_mm_copy(tmp+32+msglen, zr, 32);

	core_algo_sm3(tmp, 64+msglen, c3);
...
{ 
	...
	core_mm_copy(key, (u8*)enkey, 16);
	core_mm_copy(iv, (u8*)enkey, 16);
...
{
/*
功能：根据用户ID及公钥，求Z�?[输入] userid�?用户ID
[输入] userid_len�?userid的字节数
[输入] xa�?公钥的X坐标
[输入] xa_len: xa的字节数
[输入] ya�?公钥的Y坐标
...
返回值：
	...
	core_algo_sm3(buf, 2+userid_len+128+32+32, z);
	//free(buf);
#if EM_000101_CONFIG_FEATURE_RAM_MANAGEMENT   
    core_platform_free(buf);
#else 	
...
*/
	...
	//buf = malloc(zlen+4);
#if EM_000101_CONFIG_FEATURE_RAM_MANAGEMENT   
	buf = core_platform_alloc(zlen+4);
#else		
	buf = MEM_CallocAlt(zlen+4,1);
#endif	
	if(buf == NULL)
		return 0;

	core_mm_copy(buf, z, zlen);
	...
	if(n != 0)
	{
		...
		buf[zlen+2] = (ct >> 8) & 0xFF;
		buf[zlen+3] = ct & 0xFF;
		core_algo_sm3(buf, zlen+4, digest);
	}
...
*/
	...
	core_mm_copy(kx2, kx2buf, 32);
	core_mm_copy(ky2, ky2buf, 32);
	*kx2len = 32;
	*ky2len = 32;


	core_mm_copy(buf, kx2buf+16, 16);
	buf[0] |= 0x80;
	...
	core_mm_copy(buf, xvbuf, 32);
	core_mm_copy(buf+32, yvbuf, 32);
	core_mm_copy(buf+64, za, 32);
	core_mm_copy(buf+96, zb, 32);
	...
	{
		core_mm_copy(buf, xvbuf, 32);
		core_mm_copy(buf+32, za, 32);
		core_mm_copy(buf+64, zb, 32);
		core_mm_copy(buf+96, kx1buf, 32);
		core_mm_copy(buf+128, ky1buf, 32);
		core_mm_copy(buf+160, kx2buf, 32);
		core_mm_copy(buf+192, ky2buf, 32);
		core_algo_sm3(buf, 32*7, sb);
		buf[0] = 0x02;
		core_mm_copy(buf+1, yvbuf, 32);
		core_mm_copy(buf+33, sb, 32);
		core_algo_sm3(buf, 65, sb);
	}

	if(xv != NULL)
	{
		core_mm_copy(xv, xvbuf, 32);
		*xvlen = 32;
#if SM2_DEBUG
#else
		enrand(xv, *xvlen);
...
*/
	...
	core_mm_copy(buf, xubuf, 32);
	core_mm_copy(buf+32, yubuf, 32);
	core_mm_copy(buf+64, za, 32);
	core_mm_copy(buf+96, zb, 32);
	...
	if((s1 != NULL) || (sa != NULL))
	{
		...
		core_mm_copy(buf+160, kx2buf, 32);
		core_mm_copy(buf+192, ky2buf, 32);
		core_algo_sm3(buf, 32*7, hash);
	}

	if(s1 != NULL)
	{
		buf[0] = 0x02;
		core_mm_copy(buf+1, yubuf, 32);
		core_mm_copy(buf+33, hash, 32);
		core_algo_sm3(buf, 65, s1);
#if SM2_DEBUG
		printf("S1: ");
		PrintBuf(s1, 32);
#endif
...
*/
...
#else
	core_mm_copy(buf, xv, xvlen);
	derand(buf, xvlen);
	bytes_to_big(xvlen, (char *)buf, x3);
	core_mm_copy(buf, yv, yvlen);
	derand(buf, yvlen);
	...
	core_algo_sm3(buf, 32*7, s2);

	buf[0] = 0x03;
	big_to_bytes(32, y3, (char *)buf+1, TRUE);
	core_mm_copy(buf+1+32, s2, 32);
	core_algo_sm3(buf, 65, s2);
#if SM2_DEBUG
	printf("s2: ");
	PrintBuf(s2, 32);
#endif
...
######################################
x86: CPU 2.8G
core_algo_sm2_sign speed:147.058824/s
core_algo_sm2_verify speed:116.414435/s
core_algo_sm2_verify OK!===1
core_algo_sm2_encrypt speed:74.404762/s
core_algo_sm2_decrypt speed:150.602410/s
core_algo_sm2_decrypt OK!
...
#if SM2_DEBUG
#else
	core_algo_sm2_keygen(xa, &kx1len, ya, &ky1len, da, &ralen);
	core_algo_sm2_keygen(xb, &kx1len, yb, &ky1len, db, &ralen);
#endif
...
int main()
{
...
#if SM2_DEBUG
	int loop = 1;
	core_algo_sm3_e(userid, 18, kAx_sm2, 32, kAy_sm2, 32, msg, 14, etx);
	printf("core_algo_sm3_e: ");
	PrintBuf(etx, 32);
	...
	core_algo_sm2_keygen(kAx_sm2, &wxlen, kAy_sm2, &wylen, kA_sm2, &privkeylen);
	core_algo_sm2_keygen(xB, &wxlen, yB, &wylen, dB, &privkeylen);
#endif
	...
	for(j=0;j<loop;j++)
		core_algo_sm2_sign(digest_sm2,32,kA_sm2,32,r_sm2,&rlen,s_sm2,&slen);

	end = clock();

	tt = (double)(end-start)/CLOCKS_PER_SEC;
	printf("core_algo_sm2_sign speed:%lf/s\n", (double)j/tt);

	start = clock();

	for(j=0;j<loop;j++)
		rv = core_algo_sm2_verify(digest_sm2,32,r_sm2,32,s_sm2,32,kAx_sm2,32,kAy_sm2,32);

	end = clock();
	tt = (double)(end-start)/CLOCKS_PER_SEC;
	printf("core_algo_sm2_verify speed:%lf/s\n", (double)j/tt);

	if(rv != 1) 
		printf("core_algo_sm2_verify error!===%d\n",rv);
	else
		printf("core_algo_sm2_verify OK!===%d\n",rv);
	
	
	start = clock();

	for(j=0;j<loop;j++)
		core_algo_sm2_encrypt(tx,19,xB,32,yB,32,etx);

	end = clock();
	tt = (double)(end-start)/CLOCKS_PER_SEC;
	printf("core_algo_sm2_encrypt speed:%lf/s\n", (double)j/tt);
...
*/	
	...
	start = clock();
	for(j=0;j<loop;j++)
		core_algo_sm2_decrypt(etx,64+19+32,dB,32,mtx);
	end = clock();
	tt = (double)(end-start)/CLOCKS_PER_SEC;
	printf("core_algo_sm2_decrypt speed:%lf/s\n", (double)j/tt);

	if(core_algo_sm2_decrypt(etx,64+19+32,dB,32,mtx) < 0)
		printf("core_algo_sm2_decrypt error!\n");
	else
		printf("core_algo_sm2_decrypt OK!\n");
