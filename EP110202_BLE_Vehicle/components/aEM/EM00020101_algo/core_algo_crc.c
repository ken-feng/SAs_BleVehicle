...
void core_algo_iso3309_crc8_special(boolean is_last, u8* iv, u8 *buf ,u32 length)
{
	...
	crc8_iv = core_algo_iso3309_crc8(crc8_iv, buf, length);
...
{
	u16 crc16_iv = core_dcm_readBig16(iv);

	crc16_iv = core_algo_iso3309_crc16(crc16_iv, buf, length);

	if (is_last)
	{
		crc16_iv ^= 0xFFFF;
	}
	core_dcm_writeBig16(iv, crc16_iv);
}
#endif
...
u32 core_algo_iso3309_crc32(u32 crc32_iv, u8 *buf ,u32 length)
{
	...
	for (i=0; i<length; i++)
	{
		...
		crc32_iv = (crc32_iv << 8) ^ crc32table[(u8)((crc32_iv >> 24) ^ input)];
	}
...
{
	u32 crc32_iv = core_dcm_readBig32(iv);

	crc32_iv = core_algo_iso3309_crc32(crc32_iv, buf, length);
