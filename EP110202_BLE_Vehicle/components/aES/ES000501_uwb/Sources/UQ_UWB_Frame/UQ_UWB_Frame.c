...
//	0x19,											// config NUM
//	0x8C, 0x00,										// lens
//	0x04, 0x01, 0x09,								// UWB Channel: CH9
//	0x05, 0x01, 0x08,								// Nr of Anchors ?
//	0x06, 0x02, 0x00, 0x00,							// Device MAC Address
//	0x09, 0x04, 0x60, 0x00, 0x00, 0x00,				// Ranging Interval = 96 ms (Any value in range [16-24480] ms)
//	0x08, 0x02, 0xB0, 0x04,							// Ranging Slot Length = 1200 RSTU = 1ms
//	0x0A, 0x04, 0x00, 0x00, 0x00, 0x00,				// STS Index: Index 0
//	0x11, 0x01, 0x01,								// Device Role = Responder
//	0x14, 0x01, 0x0A,								// Preamble ID = 10
//	0x15, 0x01, 0x00,								// SFD ID = 0
//	0x1B, 0x01, 0x0C,								// Slots per Ranging Round = 12
//	0x2A, 0x02, 0x00, 0x00,							// Max RR Retry
//	0x2C, 0x01, 0x00,								// Hopping Mode = enable hopping
//	0x23, 0x01, 0x03,								// Key Rotation = STS key
//	0x32, 0x02, 0xFF, 0xFF,							// Max Block Number
//	0xE9, 0x12, 0x04, 0x00, 0x49, 0x43, 0x54, 0x53, 0x53, 0x54, 0x41, 0x54, 0x49, 0x43, 0x54, 0x53, 0x53, 0x54, 0x41, 0x54,		// D_URSK
//	0xE9, 0x12, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x49, 0x43, 0x54, 0x53, 0x53, 0x54, 0x41, 0x54,		// Salted Hash
//	0xF2, 0x01, 0x14,								// Tx Power: 14dBm-(0x14*0.25)=9dBm
//	0xA0, 0x10, 0x73, 0x05, 0x8F, 0xE5, 0x50, 0x3C, 0x1D, 0x4A, 0x17, 0x54, 0x1D, 0x51, 0x21, 0xBB, 0x7F, 0x9E,
//	0xA4, 0x02, 0x00, 0x00,							// UWB Config ID
//	0xA5, 0x01, 0x11,								// Pulse Shape Combo = 0x1 0x1
//	0xA6, 0x02, 0xD0, 0x02,							// URSK_TTL = 720 minutes
//	0xF0, 0x01, 0x00,								// N = 0
//	0xFB, 0x04, 0x02, 0x03, 0x12, 0x13,				// Rx Radio Configuration Indices = 0x02 + 0x03, Tx Radio Configuration Indices = 0x12 + 0x13
//	0xFD, 0x02, 0xFF, 0x01,							// Crypto Key Usage Flag = 0x01FF
//	0xF9, 0x01,0X01									// Enable sts index restart from 0
//};
	...
	if (UWB_Err_Success_0 == ResCode)
	{
		...
		if (UWB_Err_Success_0 == ResCode)
		{
			...
			const uint8_t DEBUG_APP_CONFIG_SET[] = {
					0x02, 0x01, 0x01,
					0x04, 0x01, 0x05,							// UWB Channel: CH9
					0x05, 0x01, 0x08,							// Nr of Anchors ?
					0x06, 0x02, 0x00, 0x01,						// Device MAC Address
					0x08, 0x02, 0xC0, 0x12,						// Ranging Slot Length = 1200 RSTU = 1ms
					//Modify (Ken):VEHICLE-V0C02 NO.3 -20231225
					0x09, 0x04, 0xC0, 0x00, 0x00, 0x00,			// Ranging Interval = 192 ms (Any value in range [16-24480] ms)
					0x0A, 0x04, 0x00, 0x00, 0x00, 0x00,			// STS Index: Index 0
					0x11, 0x01, 0x00,							// Device Role = Responder
					0x14, 0x01, 0x0A,							// Preamble ID = 10
					0x15, 0x01, 0x00,							// SFD ID = 0
					0x1B, 0x01, 0x0C,							// Slots per Ranging Round = 12
					0x1E, 0x01, (0x01 + UWB_ANCHOR_INDEX),		// Responder Slot Index = 3
					0x23, 0x01, 0x03,							// Key Rotation = STS key
					0x25, 0x01, 0x60,							//
					0x2A, 0x02, 0x00, 0x00,						// Max RR Retry
					0x2C, 0x01, 0x00,							// Hopping Mode = enable hopping
					...
					//Modify (Ken):VEHICLE-V0C02 NO.3 -20231225
					0xF2, 0x01, 0x14,							// Tx Power: 14dBm-(0x14*0.25)=9dBm
					0xF9, 0x01, 0x00,							// Enable sts index restart from 0
					0xFB, 0x04, 0x02, 0x03, 0x12, 0x13,			// Rx Radio Configuration Indices = 0x02 + 0x03, Tx Radio Configuration Indices = 0x12 + 0x13
					0xFD, 0x02, 0xFF, 0x01,						// Crypto Key Usage Flag = 0x01FF
			};
