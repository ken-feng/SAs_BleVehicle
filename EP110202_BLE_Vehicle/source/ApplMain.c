...
{
	//Modify (Ken):BleFob-V0501 NO.1 -20231205
	#if defined __FIT_Aeon_H
    ...
    LOG_L_S(BLE_MD,"\n");
    LOG_L_S(BLE_MD,">>---------------------------------------------------\n");
    LOG_L_S(BLE_MD,"Welcome Enter Application\n");
    LOG_L_S(BLE_MD,"build in (%s %s)\n", __DATE__, __TIME__);
    LOG_L_S(BLE_MD,"Project Version: EP11020302_BLE_Vehicle: %.2x%.2x \n",APP_VERSION_H,APP_VERSION_L);
    LOG_L_S(BLE_MD,"Log level is : L.%d\n", LOG_LEVEL);
    LOG_L_S(BLE_MD,">>---------------------------------------------------\n");
    LOG_L_S(BLE_MD,"\n");
...
#endif
        /*初始使用快速广播模式*/
        BleApp_SwitchAdvMode(fastAdvState_c);

        KW36_RTC_Config ();
           
        // ble_ccc_queue_init();
        Board_Config ();/*初始化所有引脚*/
...
static void App_Idle_Task(osaTaskParam_t argument)
{
    for (;;)
    {
        ...
        if (size < 0x400)
        {
            LOG_L_S(IDLE_MD, "-------Warning----------\n", size);
            LOG_L_S(IDLE_MD, "------------------------\n", size);
            size = xPortGetFreeHeapSize();
            LOG_L_S(IDLE_MD, "Heap Mini Free Size=[%d]\n", size);
            LOG_L_S(IDLE_MD, "------------------------\n", size);
            LOG_L_S(IDLE_MD, "------------------------\n", size);
            while (1);
        }
        ...
        {
            if ( ((TMR_GetTimestamp() - count) / 1000) > 60000 )
            {
                count = TMR_GetTimestamp();
                LOG_L_S(IDLE_MD, "Heap Free Size=[%d] Mini Size=[%d]\n", size, xPortGetMinimumEverFreeHeapSize());
            }
        }
...
static void App_HandleHostMessageInput(appMsgFromHost_t* pMsg)
{
    switch ( pMsg->msgType )
    {
        ...
        {
        	//LOG_L_S(BLE_MD,"MsgType:%02x  gAppGapGenericMsg_c:%02x \r\n",pMsg->msgType,pMsg->msgData.genericMsg.eventType);
        ...
        {
            LOG_L_S(BLE_MD,"MsgType:%02x  gAppGapAdvertisementMsg_c:%02x \r\n",pMsg->msgType,pMsg->msgData.advMsg.eventType);
        ...
        {
            //LOG_L_S(BLE_MD,"MsgType:%02x  gAppGapScanMsg_c:%02x \r\n",pMsg->msgType,pMsg->msgData.scanMsg.eventType);
        ...
        {
            LOG_L_S(BLE_MD,"MsgType:%02x  gAppGapConnectionMsg_c:%02x \r\n",pMsg->msgType,pMsg->msgData.connMsg.connEvent.eventType);
        ...
        {
            LOG_L_S(BLE_MD,"MsgType:%02x  gAppGattServerMsg_c:%02x \r\n",pMsg->msgType,pMsg->msgData.gattServerMsg.serverEvent.eventType);
        ...
        {
            LOG_L_S(BLE_MD,"MsgType:%02x  gAppGattClientProcedureMsg_c:%02x \r\n",pMsg->msgData.gattClientProcMsg.procedureType);
        ...
        {
            LOG_L_S(BLE_MD,"MsgType:%02x  gAppGattClientNotificationMsg_c:%02x \r\n",pMsg->msgData.gattClientNotifIndMsg.characteristicValueHandle);
        ...
        {
            LOG_L_S(BLE_MD,"MsgType:%02x  gAppGattClientIndicationMsg_c:%02x \r\n",pMsg->msgData.gattClientNotifIndMsg.characteristicValueHandle);
        ...
        {
            LOG_L_S(BLE_MD,"MsgType:%02x  gAppL2caLeDataMsg_c:%02x \r\n",pMsg->msgData.l2caLeCbDataMsg.channelId);
            // LOG_L_S_HEX(BLE_MD,"L2caLeData:",pMsg->msgData.l2caLeCbDataMsg.aPacket,pMsg->msgData.l2caLeCbDataMsg.packetLength);
        ...
        {
            LOG_L_S(BLE_MD,"MsgType:%02x  gAppL2caLeControlMsg_c:%02x \r\n",pMsg->msgData.l2caLeCbControlMsg.messageType);
...
/* On KW37, KW38, KW39 series, key handle is processed directly in the lowpower FWK module */
        ...
        // }
        // else 
        if(wakeupReason.Bits.FromCan != 0U)
        {
            Board_Set_WakeupSource (WAKEUP_SRC_CAN);
            Board_LowPower_Recovery ();
        }
